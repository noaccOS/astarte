...
defmodule Astarte.AppEngine.API.Device do
  ...
  def merge_device_status(realm_name, encoded_device_id, device_status_merge) do
    ...
    with {:ok, device_id} <- Device.decode_device_id(encoded_device_id),
         ...
         status = DeviceStatus.from_device(device, realm_name),
         {:ok, device_status} <-
           update_device_status(realm_name, device, status, device_status_merge) do
      {:ok, device_status}
    end
  end

  defp update_device_status(realm_name, device, status, device_status_merge) do
    keyspace = Realm.keyspace_name(realm_name)
    aliases = device_status_merge["aliases"]
    attributes = device_status_merge["attributes"]

    with {:ok, aliases} <- Aliases.validate(aliases, realm_name, device),
